...
function NewVosem(Candidates, foreground = 0x002F55, background = 0xeaeaea, showlabels = true, holdcross = false, WIDTH = 528, HEIGHT = 528) {
    ...
    var cross = function(size, x, y, z) {
        ...
        if (checkforview != "side")
            geometry.vertices.push( new THREE.Vector3(x, y, -(dimension / 2)), new THREE.Vector3(x, y, dimension / 2));
        if (checkforview != "top")
            geometry.vertices.push( new THREE.Vector3(x, -(dimension / 2), z),new THREE.Vector3(x, (dimension / 2), z));
        geometry.vertices.push(
    ...
    var orbitcheck = function() {
        ...
        if (polar > 1.45 && polar < 1.65 && azimuthal > -0.15 && azimuthal < 0.15 && flatten) {
            ...
            for (i = 0; i < points.length; i++) {
                new TWEEN.Tween(points[i].position).to({
                    z: 256
                }, 1000).easing(TWEEN.Easing.Exponential.Out).start();
            }
            for (i = 0; i < crosshairs.length; i++) {
                new TWEEN.Tween(crosshairs[i].scale).to({
                    z: 0.0001
                }, 600).easing(TWEEN.Easing.Exponential.Out).start();
                new TWEEN.Tween(crosshairs[i].position).to({
                    z: 256
                }, 600).easing(TWEEN.Easing.Exponential.Out).start();
            }
            new TWEEN.Tween(frame.scale).to({
                z: 0.0001
            }, 600).easing(TWEEN.Easing.Exponential.Out).start();
            new TWEEN.Tween(back.scale).to({
                x: 1.4,
                y: 1.4
            }, 600).easing(TWEEN.Easing.Exponential.Out).start();
            new TWEEN.Tween(back.material).to({
                opacity: 0.5
            }, 600).easing(TWEEN.Easing.Exponential.Out).start();
            new TWEEN.Tween(frame.position).to({
                z: 256
            }, 600).easing(TWEEN.Easing.Exponential.Out).start();
            new TWEEN.Tween(back.position).to({
                z: 256
            }, 600).easing(TWEEN.Easing.Exponential.Out).start();
            currentBGtween = new TWEEN.Tween(bottom.material).to({
                opacity: 0
            }, 100).start();
            ...
            new TWEEN.Tween(bottom.position).to({
                y: 256
            }, 600).easing(TWEEN.Easing.Exponential.Out).start();

            currentBGtween = new TWEEN.Tween(back.material).to({
                opacity: 0
            }, 100).start();
            new TWEEN.Tween(bottom.scale).to({
                x: 1.4,
                y: 1.4
            }, 600).easing(TWEEN.Easing.Exponential.Out).start();
            new TWEEN.Tween(bottom.material).to({
                opacity: 0.5
            }, 600).easing(TWEEN.Easing.Exponential.Out).start();
        } else if (polar > 3.09) {
    ...
    var moved = function() {
        if (checkforview != undefined) {
            ...
            if (checkforview == "side" && (polar < 1.45 || polar > 1.64 || azimuthal < -0.15 || azimuthal > 0.15) || checkforview_override == "side") {
                ...
                new TWEEN.Tween(top.material).to({
                    opacity: 1
                }, 100).start();
    ...
    var AddPoint = function(label, x, y, z, scene, color) {
        ...
        domEvents.addEventListener(mesh, 'mouseover', function(event) {
            if (!holdcross) {
                ...
                crosshair = new THREE.LineSegments(crosshairG, new THREE.LineBasicMaterial({
                    color: color,
                    linewidth: 1,
                    depthWrite: false,
                    depthTest: false,
                ...
                scene.add(crosshair);
                new TWEEN.Tween(crosshair.scale).to({
                    x: 1,
                    y: 1,
                    z: 1
                }, 300).easing(TWEEN.Easing.Exponential.Out).start();
                new TWEEN.Tween(crosshair.position).to({
                    x: 0.001,
                    y: 0.001,
                    z: 0.001
                }, 300).easing(TWEEN.Easing.Exponential.Out).start();
    ...
    var object = new THREE.LineSegments(geometryCube, new THREE.LineBasicMaterial({
        color: foreground,
        linewidth: 1
    }));
    frame = object;
    ...
    $("#cubeloader_wrap").animate("opacity", 0, 400, function() {
        $("#cubeloader_wrap").remove();
    });
    ...
    $("#settings-icon").on("click", function() {
        sidebtnclick("#settings-flyout", "#settings-icon");
    });
    $("#help-icon").on("click", function() {
        sidebtnclick("#help-flyout");
    });
    var sidebtnclick = function(flyout, icon = undefined) {
        var flyout = $(flyout);
        if (flyout.css("visibility") == "visible") {
            if (icon !== undefined) $(icon).css({
            ...
            flyout.css({
                opacity: 0,
                transform: "translateX(12px) scaleY(0.9)",
                visibility: "hidden"
            });
            flyout_hidden = true;
    ...
    $("#cb-flatten").click(function() {
        if ($("#cb-flatten:checked").length > 0) {
            ...
            flatten = false;
            checkforview_override = checkforview;
            moved();
        }
    })
    ...
    $("#projbtn_o").on("click", function() {
        $("#projbtn_o").attr('class', 'projpick-select');
        $("#projbtn_p").attr('class', 'projpick-unselect');
        camera.zoom = 1.75;
    ...
    $("#projbtn_p").on("click", function() {
        ...
        $("#projbtn_p").attr('class', 'projpick-select');
        $("#projbtn_o").attr('class', 'projpick-unselect');
    });
}